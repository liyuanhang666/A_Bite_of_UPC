  ...
  <div class="main-container">
    <div class="mainpage-container">
      <div class="menu-container">
        <div class="menu" v-show="beforeselect">
          {{menu_info}}
        </div>
        {{menu.name}}
      </div>
      <div class="tip">(以上菜名仅供参考XD)</div>
      <button class="click2select" @click="Toselect" v-show="beforeselect">
        点击生成今日菜单
      </button>
      <button class="click2select" @click="Toselect" v-show="selecting">
        {{button_info}}
      </button>
      <router-link :to="{path:'/food',query: {foodid: foodid}}">
        <button class="toFood click2select" v-show="isfinished">就吃这个了！</button>
      </router-link>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    mounted() {
      document.title = '今天吃点啥'
      this.foods = this.menu_info.split(',')
    },
    methods: {
      Toselect () {
        let vm = this
        ...
        if (this.rolling === -1) {
          ...
          let url = ''
          if (this.rollingtime%2 === 1) {
            url = 'http://yb.upc.edu.cn:8084/foodshare/food/latestCommend'
          }else if (this.rollingtime%2 === 0) {
            url = 'http://yb.upc.edu.cn:8084/foodshare/food/randomCommend'
          }
          this.$axios.get(url, {
            params: {
            ...
            console.log(rsp)
            this.menu.name = rsp.data.name
            this.isfinished = true
            this.button_info = '不满意，再来一次！'
            this.foodid = rsp.data.id
            let foodinfo = [rsp.data]
            let str = JSON.stringify(foodinfo)
